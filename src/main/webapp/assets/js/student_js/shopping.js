class ShoppingCart {
    ...
    bindEvents() {
        ...
        if (applyDiscountBtn) {
            applyDiscountBtn.addEventListener("click", () => this.debounce(this.applyDiscount.bind(this), 500));
        }
    ...
    displayCartItems(cart, items) {
        ...
        if (cart) {
            if (cartTotal) cartTotal.textContent = this.formatCurrency(cart.totalAmount || 0);
            if (subTotal) subTotal.textContent = this.formatCurrency(this.calculateSubTotal(items));
            if (discountAmount) discountAmount.textContent = this.formatCurrency(cart.discountAmount || 0);
        }
    }

    calculateSubTotal(items) {
        return items.reduce((total, item) => total + (item.priceAtTime * item.quantity - (item.discountApplied || 0)), 0);
    }

    createCartItemHTML(item) {
        const imageUrl = item.courseImageUrl || `${this.contextPath}/assets/img/img_student/default_course.jpg`;
        const finalPrice = item.priceAtTime * item.quantity - (item.discountApplied || 0);
        const escapedTitle = this.escapeHTML(item.courseTitle);
        const escapedDescription = this.escapeHTML(item.courseDescription || "Khóa học tiếng Nhật chất lượng cao");

        return `
            <div class="cart-item flex items-center py-6 border-b border-gray-200 transition-opacity duration-300" data-item-id="${item.cartItemID}">
                ...
                <div class="ml-6 flex-1">
                    ...
                    <div class="flex items-center space-x-4">
                        <div class="flex items-center">
                            <label class="text-sm text-gray-500 mr-2" for="quantity-${item.cartItemID}">Số lượng:</label>
                            <div class="flex items-center border border-gray-300 rounded">
                                ...
                                <span class="px-3 py-1 text-center min-w-[40px]" id="quantity-${item.cartItemID}">${item.quantity}</span>
                        ...
                        ${
                            ...
                            ? `
                                <div class="text-sm">
                                    <span class="text-gray-500 line-through">${this.formatCurrency(item.priceAtTime * item.quantity)}</span>
                            ...
                            `
                            : `
                                <div class="text-lg font-semibold text-gray-900">${this.formatCurrency(item.priceAtTime * item.quantity)}</div>
                            `
                        }
                    </div>
                </div>
        ...
        try {
            ...
            if (data.success) {
                ...
                if (item) {
                    ...
                    item.totalPrice = item.priceAtTime * newQuantity - (item.discountApplied || 0);
                }
                await this.loadCartData(false);
    ...
    async removeItem(cartItemID) {
        ...
        try {
            ...
            if (data.success) {
                this.cartData.items = this.cartData.items.filter((i) => i.cartItemID !== cartItemID);
                await this.loadCartData(false);
        ...
        if (urlParams.has('error')) {
            ...
            switch (error) {
                case "empty_cart":
                    ...
                    break;
                case "payment_failed":
                    message += `Thanh toán thất bại. Mã giao dịch: ${transactionID || "N/A"}.`;
                    break;
                case "payment_cancelled":
                    message += `Thanh toán đã bị hủy. Mã giao dịch: ${transactionID || "N/A"}.`;
                    break;
                case "partial_enrollment":
                    message += `Không thể ghi danh một số khóa học: ${failedCourses || "N/A"}. Vui lòng liên hệ hỗ trợ.`;
                    break;
                ...
                case "cart_clear_failed":
                    message += "Thanh toán thành công nhưng không thể xóa giỏ hàng. Vui lòng liên hệ hỗ trợ.";
                    break;
                default:
                    message += error;
            }
            ...
            this.showMessage("Thanh toán thành công! Các khóa học đã được ghi danh.", "success");
            window.history.replaceState({}, document.title, window.location.pathname);
        }
    }
    ...
    updateCartIcon() {
        ...
        if (cartBadge && this.cartData.cart) {
            ...
            cartBadge.style.display = this.cartData.cart.itemCount > 0 ? "flex" : "none";
        }
    }
}
...
window.cart = {
    updateQuantity: (cartItemID, quantity) => cart.debounce(cart.updateQuantity.bind(cart), 500)(cartItemID, quantity),
