...
function closeModal() {
    const modal = document.getElementById('assignmentResultModal');
    if (modal) {
...
function submitAssignment(assignmentId) {
    ...
    const enrollmentId = document.getElementById('studentEnrollmentId').value; // Bổ sung dòng này

    const submitData = {
        assignmentId: assignmentId,
        courseId: courseId,
        enrollmentId: enrollmentId, // Bổ sung dòng này
        answers: answers
    };
    ...
    fetch(`${getContextPath()}/submitAssignment`, {
        ...
        body: JSON.stringify(submitData)
            ...
            .then(result => {
                ...
                if (modalScore) {
                    modalScore.textContent = result.score || 'Chưa có điểm';
                }
                const modal = document.getElementById('assignmentResultModal');
                if (modal) {
                ...
                // Vô hiệu hóa các input radio
                document.querySelectorAll('#assignmentQuestionsList input[type="radio"]').forEach(radio => {
                    radio.disabled = true;
                });
                // Ẩn nút nộp bài
                const submitBtn = document.getElementById('submitAssignmentBtn');
                ...
                updateAssignmentProgress(assignmentId, studentId, enrollmentId, 'complete', result.score || 0);
            })
            .catch(error => {
...
// Cập nhật hàm updateAssignmentProgress
function updateAssignmentProgress(assignmentId, studentId, enrollmentId, completionStatus, score) {
    fetch('/Hikari/updateProgress', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
        body: JSON.stringify({
            studentId: studentId,
            enrollmentId: enrollmentId, // Gửi enrollmentId
            lessonId: 0,
            assignmentId: assignmentId,
            ...
            .then(data => {
                if (data.success) {
                    console.log('Cập nhật tiến độ bài tập thành công:', data.message);
                    // Cập nhật trạng thái 'completed' trên giao diện người dùng
                    const assignmentItem = document.querySelector(`.assignment-item[onclick="showAssignment(${assignmentId})"]`);
                    if (assignmentItem) {
                        const statusDiv = assignmentItem.querySelector('.lesson-status'); // Sử dụng lại class lesson-status
                        if (statusDiv) {
                            statusDiv.classList.add('completed');
                        }
                    }
                } else {
                    console.error('Lỗi cập nhật tiến độ bài tập:', data.message);
                }
            })
            .catch(error => {
                console.error('Lỗi mạng khi cập nhật tiến độ bài tập:', error);
            });
}
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function updateLessonProgress(lessonId, studentId, enrollmentId, completionStatus, score) {
        fetch('/Hikari/updateProgress', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            ...
            body: JSON.stringify({
                ...
                .then(data => {
                    if (data.success) {
                        console.log('Cập nhật tiến độ bài học thành công:', data.message);
                        const lessonItem = document.querySelector(`.lesson-item[onclick="showLesson(${lessonId})"]`);
                        if (lessonItem) {
                            const statusDiv = lessonItem.querySelector('.lesson-status');
                            statusDiv.classList.add('completed');

                        } else {
                            console.error("Không tìm thấy lessonItem cho lessonId:", lessonId);
                        }
                    } else {
                        console.error('Lỗi cập nhật tiến độ bài học:', data.message);
                    }
                })
                .catch(error => {
                    console.error('Lỗi mạng khi cập nhật tiến độ bài học:', error);
                });
    }

    const lessonVideoPlayer = document.getElementById('lessonVideoPlayer');
    if (lessonVideoPlayer) {
        lessonVideoPlayer.addEventListener('ended', () => {
            const currentLessonId = document.getElementById('lessonId').value;
            const studentId = document.getElementById('currentStudentId').value;
            const enrollmentId = document.getElementById('studentEnrollmentId').value;

            if (currentLessonId && studentId && enrollmentId) {
                updateLessonProgress(currentLessonId, studentId, enrollmentId, 'complete', 0);
            } else {
                console.warn("Không thể đánh dấu bài học hoàn thành: thiếu Lesson ID, Student ID, hoặc Enrollment ID.");
            }
        });
    }
